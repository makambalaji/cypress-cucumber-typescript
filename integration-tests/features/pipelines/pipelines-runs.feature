Feature: Pipeline Runs
    ...
    Scenario: Actions on Pipeline Run Details page : P-06-TC06
        Given pipeline run is displayed for "nodejs-ex-git" without resource
        When user clicks pipeline run of pipeline "nodejs-ex-git"
    ...
    Scenario: Rerun the Pipeline Run from pipeline run details page: P-06-TC01
        Given pipeline run is displayed for "nodejs-ex-git" without resource
        And user clicks pipeline run of pipeline "nodejs-ex-git"
    ...
    @regression, @manual
    Scenario: Download the logs from Pipeline Details page : P-04-TC05
    ...
    @regression, @manual
    Scenario: Download the logs from Pipeline Details page : P-05-TC06
    ...
    Scenario: Start LastRun from topology page : P-05-TC04
        ...
        And user clicks Start LastRun from topology side bar
    ...
    Scenario: Maximum pipeline runs display in topology page: P-05-TC05
        Given workload "nodejs-ex-git" is created from add page with pipeline
        And user is at pipelines page
        And pipeline "nodejs-ex-git" is executed for 5 times
