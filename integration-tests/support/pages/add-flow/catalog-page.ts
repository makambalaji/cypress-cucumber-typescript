...
export const catalogPage = {
  ...
  isCheckBoxSelected: (type: string) =>
    cy.get(`input[title="${type}"]`).should("be.checked"),
  isCardsDisplayed: () => cy.get(catalogPageObj.card).should("be.visible"),
  search: (keyword: string) =>
    cy.get(catalogPageObj.search).clear().type(keyword),
  verifyDialog: () =>
    cy.get(catalogPageObj.sidePane.dialog).should("be.visible"),
  verifyInstallHelmChartPage: () =>
    cy.get("form h1").eq(0).should("have.text", "Install Helm Chart"),
  ...
  selectHelmChartCard: (cardName: string) =>
    cy.get("a div.catalog-tile-pf-title").contains(cardName).click(),
