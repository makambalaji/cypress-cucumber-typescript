...
export const pipelineDetailsPage = {
  verifyTitle: (pipelineName: string) =>
    cy.pageTitleShouldContain(pipelineName),
  ...
  verifyTriggerTemplateSection: () =>
    cy
      .get(pipelineDetailsObj.details.triggerTemplateSection)
      .should("be.visible"),
  verifyPage: () =>
...
export const triggerTemplateDetailsPage = {
  verifyPage: () =>
    cy
      .get(triggerTemplateDetailsObj.title)
      .should("contain.text", "Trigger Template Details"),
  verifyTabs: () => {
  ...
  verifyActionsDropdown: () =>
    cy.get(triggerTemplateDetailsObj.actions).should("be.visible"),
  selectEventListener: () =>
    cy.get(triggerTemplateDetailsObj.details.eventListenerLink).click(),
};

export const eventListenerDetailsPage = {
  verifyPage: () =>
    cy
      .get(eventListenerDetailsObj.title)
      .should("contain.text", "Event Listener Details"),
  verifyTabs: () => {
    cy.get("ul.co-m-horizontal-nav__menu li a").as("tabName");
    cy.get("@tabName").eq(0).should("have.text", "Details");
    cy.get("@tabName").eq(1).should("have.text", "YAML");
  ...
  verifyActionsDropdown: () =>
    cy.get(eventListenerDetailsObj.actions).should("be.visible"),
  selectTriggerBindingLink: () =>
    cy.get(eventListenerDetailsObj.details.triggerBindingLink).click(),
};

export const clusterTriggerBindingDetailsPage = {
  verifyPage: () =>
    cy
      .get(clusterTriggerBindingDetailsObj.title)
  ...
  verifyFields: () => {
    cy.get('[data-test-id="resource-summary"] dt .details-item__label').as(
      "fieldNames"
    );
    cy.get("@fieldNames").eq(0).should("have.text", "Name");
  ...
  verifyActionsDropdown: () =>
    cy.get(clusterTriggerBindingDetailsObj.actions).should("be.visible"),
};
