...
export const pipelineRunDetailsPage = {
  verifyTitle: () =>
    cy
      .get(pipelineRunDetailsObj.details.sectionTitle)
      .should("have.text", "Pipeline Run Details"),
  waitForTaskRunToComplete: () => {
    pipelineRunDetailsPage.waitForTaskRunToComplete();
  },
  verifyPipelineRunStatus: (status: string) =>
    cy.get(pipelineRunDetailsObj.pipelineRunStatus).should("have.text", status),
  fieldDetails: (fieldName: string, expectedFieldValue: string) =>
    cy
      .get(pipelineRunDetailsObj.details.pipelineRunDetails)
      .contains(fieldName)
      .next("dd")
  ...
  verifyActionsDropdown: () =>
    cy.get(pipelineRunDetailsObj.actions).should("be.visible"),
  selectPipeline: () =>
    cy.get(pipelineRunDetailsObj.details.pipelineLink).click(),
...
export const pipelineRunsPage = {
  ...
  search: (pipelineRunName: string) =>
    cy.byLegacyTestID("item-filter").type(pipelineRunName),
  selectKebabMenu: (pipelineRunName: string) => {
    cy.get(pipelineRunsObj.pipelineRunsTable.table).should("exist");
    cy.get(pipelineRunsObj.pipelineRunsTable.pipelineRunName).each(
      ($el, index) => {
        const text = $el.text();
  ...
  verifyPipelineRunsTableDisplay: () =>
    cy.get(pipelineRunsObj.pipelineRunsTable.table).should("be.visible"),
  ...
  verifyStatusInPipelineRunsTable: (status: string) => {
    cy.get(pipelineRunsObj.pipelineRunsTable.status).should(
      "have.text",
      status
    );
  },
