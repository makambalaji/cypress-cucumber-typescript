...
export const pipelinesPage = {
  ...
  seelctPipelineRun: (pipelineName: string) => {
    cy.get(pipelinesObj.pipelinesTable.table, { timeout: 30000 }).should(
      "exist"
    );
    cy.get(pipelinesObj.pipelinesTable.pipelineName).each(($el, index) => {
  ...
  verifyCreateButtonIsEnabled: () =>
    cy.get(pipelinesObj.createPipeline).should("be.enabled"),

  verifyKebabMenu: () =>
    cy.get(pipelinesObj.pipelinesTable.kebabMenu).should("be.visible"),
  ...
  verifyLastRunStatusInPipelinesTable: (lastRunStatus: string) => {
    cy.get("tbody td:nth-child(4) span span").should(
      "have.text",
      lastRunStatus
    );
  },
...
export const startPipelineInPipelinsPage = {
  ...
  addSecret: (
    ...
    cy.get(pipelinesObj.startPipeline.advancedOptions.secretName).type(
      secretName
    );
    cy.selectByDropDownText(
      pipelinesObj.startPipeline.advancedOptions.accessTo,
      provider
    );
    cy.get(pipelinesObj.startPipeline.advancedOptions.serverUrl).type(
      serverUrl
    );
    cy.selectByDropDownText(
      pipelinesObj.startPipeline.advancedOptions.authenticationType,
      authenticationType
    );
  ...
  verifyCreateSourceSecretSection: () =>
    cy
      .get(pipelinesObj.startPipeline.advancedOptions.secretFormTitle)
      .should("be.visible"),
  verifyFields: () => {
