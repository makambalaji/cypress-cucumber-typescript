...
export const pipelineBuilderPage = {
  ...
  verifyDefaultPipelineName: (pipelineName: string = "new-pipeline") =>
    cy.get(pipelineBuilderObj.name).should("have.value", pipelineName),
  ...
  addResource: (resourceName: string, resourceType: string = "Git") => {
    ...
    cy.selectByDropDownText(
      pipelineBuilderObj.addResources.resourceType,
      resourceType
    );
  },
  verifySection: () => {
  ...
  createPipelineFromYamlPage: () => {
    ...
    cy.get(pipelineBuilderObj.switchToYamlEditorAlert.alertDialog).should(
      "be.visible"
    );
    cy.get(pipelineBuilderObj.switchToYamlEditorAlert.title).should(
      "contain.text",
      "Switch to YAML Editor?"
    );
    cy.get(pipelineBuilderObj.switchToYamlEditorAlert.continue).click();
    cy.get(pipelineBuilderObj.yamlCreatePipeline.helpText).should(
      "contain.text",
      "YAML or JSON"
    );
