...
export const operatorsObj = {
  ...
  installOperators: {
    ...
    operatorsNameRow:
      'div[aria-label="Installed Operators"] td:nth-child(1) h1',
...
export const operatorsPage = {
  ...
  installOperator: () => {
    cy.get(operatorsObj.installOperators.title).should(
      "have.text",
      "Install Operator"
    );
  ...
  verifyInstalledOperator: (operatorName: string) => {
    cy.get(operatorsObj.installOperators.search)
      .should("be.visible")
      .clear()
      .type(operatorName);
  ...
  selectOperator: (opt: operators | string) => {
    switch (opt) {
      ...
      case operators.serverlessOperator: {
        cy.byTestID(
          "serverless-operator-redhat-operators-openshift-marketplace"
        ).click();
        break;
      }
      ...
      case operators.virtualizationOperator: {
        cy.byTestID(
          "kubevirt-hyperconverged-redhat-operators-openshift-marketplace"
        ).click();
        break;
      }
      ...
      case operators.knativeCamelOperator: {
        cy.byTestID(
          "knative-camel-operator-community-operators-openshift-marketplace"
        ).click();
      ...
      case operators.eclipseCheOperator: {
        cy.byTestID(
          "eclipse-che-community-operators-openshift-marketplace"
        ).click();
        cy.alertTitleShouldContain("Show Community Operator");
        cy.byTestID("confirm-action").click();
        break;
