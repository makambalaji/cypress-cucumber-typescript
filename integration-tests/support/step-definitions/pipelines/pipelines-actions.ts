...
Given(
  "pipeline {string} is present on Pipelines page",
  (pipelineName: string) => {
    pipelinesPage.clickOncreatePipeline();
    pipelineBuilderPage.createPipelineFromBuilderPage(pipelineName);
...
When(
  "user clicks pipeline name {string} on Pipelines page",
  (pipelineName: string) => {
...
When(
  "user clicks pipeline run of pipeline {string}",
  (pipelineName: string) => {
...
When(
  "user clicks kebab menu for the pipeline {string}",
  (pipelineName: string) => {
    pipelinesPage.selectKebabMenu(pipelineName);
  }
);
...
Then(
  "kebab menu displays with options Start, Add Trigger, Edit Pipeline, Delete Pipeline",
  () => {
    cy.byTestActionID("Start").should("be.visible");
    cy.byTestActionID("Add Trigger").should("be.visible");
    cy.byTestActionID("Edit Pipeline").should("be.visible");
...
Then(
  "user is able to see Details, YAML, Pipeline Runs, Parameters and Resources tabs",
  () => {
    // TODO: implement step
  }
);
...
Then(
  "Actions menu display with options {string}, {string}, {string}, {string}",
...
Then(
  "pipeline run status displays as {string} in Pipeline run page",
  (status: string) => {
    cy.log(status);
  }
);
