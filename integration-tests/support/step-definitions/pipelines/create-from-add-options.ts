...
Then(
  "pipeline section is displayed with message {string}",
  (message: string) => {
...
When(
  "user enters Git Repo url in docker file as {string}",
  (gitRepoUrl: string) => {
...
When(
  "user enters Git Repo url in builder image as {string}",
  (gitRepoUrl: string) => {
    addPage.enterGitUrl(gitRepoUrl);
    addPage.verifyValidatedMessage();
  }
...
When(
  "user searches for pipeline {string} in pipelines page",
  (name: string) => {
...
Then(
  "pipeline name {string} is displayed in topology side bar",
  (appName: string) => {
...
Then(
  "pipeline {string} is displayed in pipelines page",
  (pipelineName: string) => {
...
Given(
  "workload {string} is created from add page with pipeline",
  (pipelineName: string) => {
    naviagteTo(menu.Add);
    addPage.selectCardFromOptions(addOptions.Git);
...
When(
  "user searches builder image {string} in developer catalog",
  (searchItem: string) => {
