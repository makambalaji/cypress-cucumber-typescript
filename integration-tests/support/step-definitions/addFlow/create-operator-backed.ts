...
When('user enters name as {string} in Create knative Serving page', (name: string) => {
  ...
  cy.get(catalogPageObj.createknativeServing.name).clear().type(`${name}-${timestamp}`);
});
...
Then('user is able to see workload {string} in topology page from knative Serving page', (name: string) => {
