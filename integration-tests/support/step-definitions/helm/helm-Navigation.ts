...
Then("release name displays as {string}", (name: string) => {
  cy.get(catalogPageObj.installHelmChart.releaseName).should(
    "have.value",
    name
  );
...
When(
  "user selects checkbox for the Deployed Helm charts",
  (workloadname: string) => {
...
When(
  "user clicks on the helm release name {string}",
...
Then(
  "user will get the link to install helm charts from developer catalog",
  () => {
...
Then(
  "helm charts with status {string} are listed",
  (helmChartstatus: string) => {}
);
...
Then(
  "user is able to see message on the Helm page as {string}",
  (message: string) => {
    cy.log(message);
  }
);
...
Then(
  "Actions menu display with options Upgrade, Rollback, and Uninstall Helm Release",
  () => {
