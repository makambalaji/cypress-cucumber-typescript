...
Given(
  "user has creaeted two workloads {string} and {string}",
  (firstWorkload: string, secondWorkload: string) => {
...
When(
  "user selects {string} option from Action menu",
  (actionMenuOption: string) => {
...
When(
  "user enters value as name of the node {string} to which it will be associated",
...
When(
  "user click on the front of arrow and drag it on to the other node and drop it",
  () => {
    // manual step
  }
);
...
Then(
  "user can see the arrow connecting them with head pointing to the node where the arrow is dropped",
  () => {
    // manual step
  }
);
