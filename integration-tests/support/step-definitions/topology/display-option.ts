...
Given(
  "user has created deployment, deployment-config and knative-service type resources",
  () => {
...
When(
  "user sees {string} and {string} under {string} and {string} have options according to their presence which are {string} and {string}",
...
When(
  "user checks the workloads which shows pod count instead of buider images",
  () => {
    // TODO: implement step
  }
);
