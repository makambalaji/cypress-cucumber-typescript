...
Given(
  "user is at Operator Hub page with the header name {string}",
  (headerName) => {
...
When(
  "user clicks OpenShift Pipelines Operator card on Operator Hub page",
  () => {
...
Then(
  "Installed operators page will contain {string}",
  (operatorName: string) => {
...
When(
  "user installs the OpenShift Serverless operator with default values",
  () => {
    operatorsPage.installOperator();
  }
);
...
Given(
  "cluster is installed with knative serverless and eventing operators",
  () => {
    operatorsPage.verifyOperatorInNavigationMenu("Serverless");
    operatorsPage.navigateToInstalloperatorsPage();
    operatorsPage.verifyInstalledOperator("OpenShift Serverless Operator");
    cy.get('a[title="knativeeventings.operator.knative.dev"]').should(
      "be.visible"
    );
  }
);
...
When(
  "user clicks unistall button present in modal with header message Uninstall Operator?",
  () => {
...
When(
  "user clicks knative eventing provided api pressent in knative serverless operator",
  () => {
...
When(
  "user clicks Create knative Eventing button present in knative Eventing tab",
  () => {
...
When(
  "user search and installs the knative Camel operator with default values",
  () => {
    operatorsPage.searchOperator("OpenShift Serverless Operator");
...
Then(
  "Installed operators page will not contain {string}",
  (operatorName: string) => {
    operatorsPage.verifyOperatoNotAvailable(operatorName);
  }
);
