...
Given(
  "knative service, event source and sink connector are present in topology page",
  () => {
...
When(
  "user selects the Delete option on {string} modal",
  (modalTitle: string) => {
...
Then(
  "event source {string} will not be displayed in topology page",
  (eventSourceName: string) => {
    naviagteTo(devNavigationMenu.Topology);
    topologyPage.search(eventSourceName);
    cy.reload();
    cy.get('[data-type="event-source"]', { timeout: 50000 }).should(
      "not.be.visible"
    );
  }
);
