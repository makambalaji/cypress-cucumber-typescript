...
When(
  "user clicks on the revision of knative service {string}",
  (serviceName: string) => {
...
When(
  "user clicks on Actions dropdown in top right corner of side bar",
  () => {}
);
...
Then(
  "side bar is displayed with heading name as {string}",
  (serviceName: string) => {
...
Then(
  "user able to see pods status as {string} by default",
  (podStatus: string) => {
    cy.log(podStatus);
  }
);
...
When(
  "user clicks on the knative service name {string}",
  (serviceName: string) => {
